...
.dms-switch {
        //...
  background: var(--neu-bg); //1️⃣ Swaps color via variable
  transition: background 0.5s, box-shadow 0.5s; //2️⃣ Background & shadow cross-fade on theme switch
}`,
};
...
.dms-scene {
        //...
  --neu-glow: 251, 191, 64; //2️⃣ RGB triplet only
}
.dms-dark {
        //...
  --neu-glow: 96, 128, 248; //2️⃣ RGB triplet only
}
.dms-glow {
        //...
  ...
  top: 10%; //3️⃣
  left: 10%; //3️⃣
  transform: translate(-50%, -50%); //3️⃣ Anchor at its own center

  background: radial-gradient(
    circle, //1️⃣ Circular diffusion
    rgba(var(--neu-glow), 0.6) 0%, //1️⃣ Brightest glow at center
    rgba(var(--neu-glow), 0.22) 45%, //1️⃣ Mid-section semi-transparent
    rgba(var(--neu-glow), 0) 75% //1️⃣ Edges fully transparent
  );
  filter: blur(8px); //1️⃣ Soften edges
  transition: left 0.45s cubic-bezier(0.34, 1.56, 0.64, 1),
              background 0.5s; //4️⃣ Glow bounces along with the knob
}

.dms-dark .dms-glow {
  top: 90%; // Glow moves to bottom-right of track
  left: 90%; // Follows the knob to the right
}
  `,
};
...
.dms-knob {
        //...
  transform: translateY(-50%); // Vertically centered in track
  border: 0.2px solid rgba(255, 150, 40, 0.4); // Thin warm-orange border
  background: radial-gradient(circle at 38% 38%, #fbbf40, #f07020); //1️⃣ Off-center sun-orange gradient, simulates single-side light
  box-shadow:
  ...
  transition: //2️⃣
    left 0.45s cubic-bezier(0.34, 1.56, 0.64, 1), // Bounce with overshoot
    transform 0.45s cubic-bezier(0.34, 1.56, 0.64, 1), // Squash in sync
    background 0.5s, // Gradient color cross-fade
    box-shadow 0.5s; // Shadow color cross-fade
}
.dms-dark .dms-knob { //3️⃣
  border: 0.2px solid rgba(60, 100, 255, 0.5); // Cool-blue border for dark theme
  background: radial-gradient(circle at 38% 38%, #6080f8, #1a30c0); // Blue moonlight gradient
  box-shadow:
    4px 4px 10px rgba(0, 0, 0, 0.45), // Deeper drop shadow (dark bg needs heavier)
    -2px -2px 7px rgba(60, 80, 180, 0.25), // Cool-blue highlight
    inset 1px 1px 3px rgba(120, 150, 255, 0.5), // Blue inner glow
    0 0 20px rgba(60, 100, 255, 0.5); // Blue outer halo
}
.dms-switch:active .dms-knob {
  transform: translateY(-50%) scaleX(1.18); //4️⃣ Squash horizontally on press
}

.dms-knob::before { //5️⃣ Close-range halo behind the knob
        //...
  inset: -10px; // Extends 10px beyond the knob
  background: radial-gradient(circle, rgba(var(--neu-glow), 0.5) 0%, rgba(var(--neu-glow), 0) 70%); // Fades from center outward
  filter: blur(4px); // Soft edges
  transition: background 0.5s; // Halo color fades with theme switch
}
    `,
  };
...
export const NEUMORPHIC_SECTION: BodySection = {
  ...
  content: {
    en: "Most of the tactile feel in this scene comes down to nothing more than which direction the shadows fall.\n1️⃣ **Inset = sunken.** Firing an `inset box-shadow` from two opposite corners at once — a dark shadow on one side, a light highlight on the other — reads as a surface that's been pressed inward, and that's exactly what `.neu-card` uses for the text block; its background is also deliberately a shade darker than the scene's base color, which deepens that sunken look.\n2️⃣ **Outward = raised.** Flip the same pair of shadows to point outward from two opposite corners instead, and the surface reads as raised rather than pressed in — that's `.neu-raised` used on the icon tiles.\nThose icon tiles then layer three separate interaction states on top:\n3️⃣ `:hover` — the element floats up 2px with an intentional shadow imbalance, meant to read as light hitting from one direction.\n4️⃣ `:active` — the element settles back toward its resting position while the whole shadow set flips to its `inset` version, so it genuinely looks pressed into the surface.\n5️⃣ `:focus-visible` — simply layers an outline ring on top while leaving the shadow direction untouched. Purely so that anyone navigating by keyboard can still see exactly which tile currently has focus.",
    "zh-TW":
...
.neu-card { //1️⃣
        //...
  background: var(--neu-surface); // Card surface, slightly darker than bg
  box-shadow:
    inset 2px 2px 5px var(--neu-surface-inset-dark), // Top-left inner shadow, sunken feel
    inset -2px -2px 5px var(--neu-surface-inset-light); // Bottom-right inner highlight
  transition: background 0.5s, box-shadow 0.5s; // Background & shadow cross-fade on theme switch
}
.neu-raised { //2️⃣
        //...
  background: var(--neu-surface); // Tile surface
  box-shadow:
    3px 3px 8px var(--neu-shadow-dark), // Bottom-right shadow, raised effect
    -3px -3px 8px var(--neu-shadow-light); // Top-left highlight, neumorphic lift
  transition: background 0.5s, box-shadow 0.5s, transform 0.25s ease;
}
.neu-raised:hover { //3️⃣
  transform: translateY(-2px); // Float up 2px
  background: var(--neu-surface-inset-light); // Brighter surface, hints selection
  color: var(--neu-focus); // Icon color switches to theme accent
  box-shadow:
    5px 5px 24px rgba(var(--neu-glow), 0.6), // Wide themed glow
    -5px -5px 24px rgba(var(--neu-glow), 0.22); // Soft counter-glow
}
.neu-raised:active { //4️⃣
  transform: translateY(0) scale(0.95); // Settles back and shrinks, pressed feel
  box-shadow:
    inset 2px 2px 12px rgba(var(--neu-glow), 0.6), // Inset glow, surface caves in
    inset -2px -2px 12px rgba(var(--neu-glow), 0.22); // Counter inset shadow
}
.neu-raised:focus-visible { //5️⃣
  box-shadow:
    3px 3px 8px var(--neu-shadow-dark), // Keep raised shadow
    -3px -3px 8px var(--neu-shadow-light), // Keep raised shadow
    0 0 0 3px var(--neu-focus); // Keyboard focus ring
}
  `,
};
