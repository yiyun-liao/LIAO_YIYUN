...
export const DARKMODESWITCHDEMO: Demo = {
  ...
  introduction: {
    ...
    refs: [
      ...
      { label: "MDN — cubic-bezier() easing function", url: "https://developer.mozilla.org/en-US/docs/Web/CSS/easing-function/cubic-bezier" },
...
export const MOTIVATION_SECTION: BodySection = {
  ...
  content: {
    ...
    "zh-TW": "還在做設計師的時候，我常跟前端工程師開玩笑說，總有一天要出一份誇張到不行的 switch 規格書整他們。他們的回答永遠一樣：那你自己做啊。所以這次真的要動手做點東西的時候，我就想，不然就從這顆 switch 開始吧。",
  },
};

// ─── 01 Anatomy of the Switch ───

export const SWITCH_SECTION: BodySection = {
...
// ─── 02 CSS Custom-Property Theming ───
...
export const CSS_THEME_SECTION: BodySection = {
  ...
  content: {
    ...
    "zh-TW": "這個專案的主角雖然是切換開關，但同時也一併建了一套完整的主題系統——這種基礎設施在過去的專案裡常常因為時間壓力被擱置，這次乾脆把它做完整。\n作法很單純：`.dms-scene` 這個最外層的容器先定義了多個 CSS 自訂屬性，當作淺色模式的調色盤；`.dms-dark` 這個 class 則把同一組變數名稱、全部重新覆寫成深色版本的數值。換句話說，明暗主題真正的差異只在於『同一批變數被指定了哪一組數值』，元件本身完全不需要知道自己現在是淺色還是深色。\nReact 那一端因此也變得很輕：只要切換 `dark` 這個布林值，決定要不要把 `dms-dark` 這個 class 名稱掛上去，剩下的全部交給 CSS cascade 去處理，而不是寫死在各自規則裡的色碼。",
  },
};

export const CSS_THEME_CODE_HTML: CodeBlock = {
  codeType: "tsx",
  code: `<div className={\`dms-scene\${dark ? " dms-dark" : ""}\`}>
  {/* ... */}
</div>`,
};

export const CSS_THEME_CODE_VARS: CodeBlock = {
  codeType: "css",
  code: `.dms-scene {
  --neu-bg: #e0e5ec;
...
export const CSS_THEME_TAKEAWAY: BodySection = {
  title: _,
  content: {
    en: "That's the part of this system that's easiest to forget and most important to get right: every themed element has to opt into its own `transition`, or the color change will snap instead of fade.",
    "zh-TW": "這也是這套主題系統最容易被忽略、卻最關鍵的一步：每一個會隨主題變色的元件，都要自己宣告該對哪些屬性做 `transition`，否則畫面就會用『跳』的方式換色，而不是『淡』過去。",
  },
};

// ─── 03 Glow Effect ───

export const GLOW_SECTION: BodySection = {
  ...
  content: {
    ...
    "zh-TW": "這個專案除了擬物化的觸感之外，另一個重點是光暈——我一直很喜歡那種表面看起來乾淨、底下卻悄悄透出一層光的介面，有層次但不會顯得雜亂。\n`.dms-glow` 整個效果其實就是一個用 `radial-gradient` 疊出同心圓漸層、再加上 `blur` 模糊邊緣的 `div`。\n決定這圈光暈顏色的是 `--neu-glow` 這個 token，但它存的不是平常習慣的 hex 色碼，而是單純一組 RGB 三元組（例如 `251, 191, 64`），這樣才能直接套進 `rgba(var(--neu-glow), 0.6)` 這種寫法、自由控制每一層的透明度——如果存成 hex 字串，就沒辦法在同一個變數上疊不同的透明度。也因為這個 token 在 `.dms-dark` 底下被重新指定成另一組數值，主題切換的那一刻，這圈光暈就自動從暖橘色變成冷藍色，完全不需要為了配色另外寫 JS 邏輯。",
  },
};

export const GLOW_CODE_RGB: CodeBlock = {
  codeType: "css",
  code: `.dms-scene {
...
export const GLOW_POSITION: BodySection = {
  title: _,
  content: {
    ...
    "zh-TW": "`.dms-glow` 本身的定位用的是一個很常見的組合：`position: absolute` 加上 `top`、`left` 設一個百分比，再用 `transform: translate(-50%, -50%)` 把錨點從預設的左上角重新拉回中心點，這樣後面只要改 `top` / `left` 的數值，移動的就會是『中心』而不是『邊角』，定位起來才符合直覺；深色模式底下把 `top` / `left` 從 `10%` 改成 `90%`，整圈光暈就會跟著旋鈕一起跑到軌道的另一端。",
  },
};

export const GLOW_CODE_POSITION: CodeBlock = {
  codeType: "css",
  code: `.dms-glow {
  position: absolute;
  top: 10%;
...
export const GLOW_TAKEAWAY: BodySection = {
  title: _,
  content: {
    en: "And it's worth not overlooking that `.dms-glow` declares its own `transition` — without which both the movement and the color change would snap instantly; the bounce and the fade you actually see both come from that one line.",
    "zh-TW": "最後 `.dms-glow` 自己也宣告了 `transition`，沒有這行光暈的位移跟變色都會是瞬間跳過去。",
  },
};

// ─── 04 The Knob Itself ───

export const KNOB_SECTION: BodySection = {
  title: { en: "The Knob Itself", "zh-TW": "旋鈕本體" },
  content: {
    en: "What actually decides what the knob feels like it's made of is the background: `radial-gradient` deliberately pushes its light source off-center to `38% 38%`, simulating something lit from one side rather than a flat, lightless color swatch. The sense of depth comes from four `box-shadow` layers stacked together.",
    "zh-TW": "決定它『看起來像什麼材質』的是 `background`：`radial-gradient` 把光源點刻意偏到左上角的，而不是均勻無光源的平面色塊。立體感則交給四層疊在一起的 `box-shadow`。",
  },
};

export const KNOB_CODE_GRADIENT: CodeBlock = {
  codeType: "css",
  code: `.dms-knob {
...
export const KNOB_TRANSITION: BodySection = {
  title: _,
  content: {
    en: "`left` animates over `0.45s` with `cubic-bezier(0.34, 1.56, 0.64, 1)` for a bounce with a slight overshoot, `transform` rides the same curve to squash in sync, and `background` and `box-shadow` each cross-fade their colors over `0.5s`. Stack all four transitions together and the knob reads as something with weight that springs into place, rather than a sprite that snaps from one side to the other.",
    "zh-TW": "`left` 用 `0.45s` 的 `cubic-bezier` 做出帶過衝的彈跳位移，`transform` 用同一條曲線同步做擠壓變形，`background` 跟 `box-shadow` 則各自用 `0.5s` 慢慢淡換顏色，四個 `transition` 疊在一起，才會看起來像一顆有重量、會彈一下的實體零件，而不是貼圖瞬間跳到另一邊。",
  },
};

export const KNOB_CODE_TRANSITION: CodeBlock = {
  codeType: "css",
  code: `.dms-knob {
  transition:
...
export const KNOB_DARK: BodySection = {
  title: _,
  content: {
    en: "Flip to dark mode and the exact same set of properties gets repainted in cool blue.",
    "zh-TW": "切到深色模式之後，同一組設定全部重新填上冷藍色版本。",
  },
};

export const KNOB_CODE_DARK: CodeBlock = {
  codeType: "css",
  code: `.dms-dark .dms-knob {
...
export const KNOB_ACTIVE: BodySection = {
  title: _,
  content: {
    en: "The instant you press it, `:active` layers on an additional `scaleX(1.18)`, squashing the knob horizontally to simulate the feel of a finger pressing down.",
    "zh-TW": "按下去的瞬間，`:active` 額外疊上一個 `scaleX(1.18)`，把旋鈕水平擠扁，模擬手指按壓的回饋感。",
  },
};

export const KNOB_CODE_ACTIVE: CodeBlock = {
  codeType: "css",
  code: `.dms-switch:active .dms-knob {
  transform: translateY(-50%) scaleX(1.18);
}`,
};

export const KNOB_BEFORE: BodySection = {
  title: _,
  content: {
    en: "And finally, `.dms-knob::before` is the small halo sitting directly behind the knob — it carries its own `transition` so that this close-in halo fades its color in step with everything else during a theme switch.",
    "zh-TW": "最後，`.dms-knob::before` 是緊貼在旋鈕正後方，而它自己也掛著 `transition`，確保主題切換時這圈貼著旋鈕的光環也會跟著一起慢慢變色。",
  },
};

// ─── 05 Neumorphic Surface Details ───

export const NEU_SECTION: BodySection = {
  ...
  content: {
    ...
    "zh-TW": "這個場景裡，大部分的觸感其實都只取決於陰影打的方向。\n`.neu-card` 從兩個相反角落同時打出 `inset box-shadow`——一邊往內陰影、一邊往內亮面——看起來就會像表面被『壓』進去一塊，背景色也特意比場景底色更深一階，加深那種凹陷感。",
  },
};

export const NEU_CODE_CARD: CodeBlock = {
  codeType: "css",
  code: `.neu-card {
...
export const NEU_RAISED: BodySection = {
  title: _,
  content: {
    en: "**Outward = raised.** Flip the same pair of shadows to point outward from two opposite corners instead, and the surface reads as raised rather than pressed in — that's `.neu-raised` used on the icon tiles.",
    "zh-TW": "把同一組陰影方向反過來，改成從兩個相反角落打出去的外陰影，表面就會反過來讀成『凸』起來的，這是 `.neu-raised` 用在圖示上的版本。",
  },
};

export const NEU_CODE_RAISED: CodeBlock = {
  codeType: "css",
  code: `.neu-raised {
  background: var(--neu-surface);
  box-shadow:
...
export const NEU_HOVER: BodySection = {
  title: _,
  content: {
    en: "Those icon tiles then layer three separate interaction states on top:\n`:hover` — the element floats up 2px with an intentional shadow imbalance, meant to read as light hitting from one direction.",
    "zh-TW": "圖示上又疊了三種互動狀態，各自做不同的事：\n`:hover` 故意做出不對稱才會有光從一側打過來的感覺。",
  },
};

export const NEU_CODE_HOVER: CodeBlock = {
  codeType: "css",
  code: `.neu-raised:hover {
...
export const NEU_ACTIVE_TEXT: BodySection = {
  title: _,
  content: {
    en: "`:active` — the element settles back toward its resting position while the whole shadow set flips to its `inset` version, so it genuinely looks pressed into the surface.",
    "zh-TW": "`:active` 按下去的時候，元素縮小並退回原位，陰影也整組換成 `inset` 版本，讓視覺上真的像被按進了表面。",
  },
};

export const NEU_CODE_ACTIVE: CodeBlock = {
  codeType: "css",
  code: `.neu-raised:active {
...
export const NEU_FOCUS_TEXT: BodySection = {
  title: _,
  content: {
    en: "`:focus-visible` — simply layers an outline ring on top while leaving the shadow direction untouched. Purely so that anyone navigating by keyboard can still see exactly which tile currently has focus.",
    "zh-TW": "`:focus-visible` 則只單純疊加一圈外框，陰影方向維持不變，純粹是為了讓用鍵盤切換焦點的人，能在不靠滑鼠的情況下，清楚看到目前選到的是哪一個。",
  },
};

export const NEU_CODE_FOCUS: CodeBlock = {
  codeType: "css",
  code: `.neu-raised:focus-visible {
  box-shadow:
    3px 3px 8px var(--neu-shadow-dark),
    -3px -3px 8px var(--neu-shadow-light),
