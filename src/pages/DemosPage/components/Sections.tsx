    ...
    <section className="wrap flex flex-col gap-16">
      {sections.map((s, i) => (
        <div key={i} className="flex flex-col m-auto max-w-[800px] justify-center ">
          ...
          <h3 className="font-serif text-[clamp(20px,3vw,32px)] leading-[1.1] tracking-[-.01em] mb-4">{l(s.title)}</h3>
          <p className="text-ink-soft text-sm leading-[1.7] mb-6">{l(s.content)}</p>
          {s.code && (
