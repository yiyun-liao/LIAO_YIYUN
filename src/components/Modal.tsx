      ...
      >
        {images.map((src, i) => (
          <img
            ...
            src={src}
            alt={`${name} ${i + 1}`}
            className="w-full h-full object-contain flex-shrink-0 bg-[#EDECE6]"
          />
        ))}
      </div>
      {images.length > 1 && (
...
export function Modal({ project: p, onClose, onPrev, onNext }: ModalProps) {
  ...
  return (
    <div className="modal-enter fixed inset-0 bg-ink/88 z-90 flex items-stretch justify-stretch p-0" onClick={onClose}>
      <div
      ...
      >
        ...
        <div className="pt-7 px-[22px] pb-12 md:pt-12 md:px-14 md:pb-[72px]">
          <div className="grid grid-cols-1 gap-3 items-start md:grid-cols-[auto_1fr_auto] md:gap-7 md:items-end border-b border-ink pb-7 mb-9">
            <div className="font-serif text-[56px] md:text-[100px] leading-[.8] font-normal tracking-[-.02em]">
              {p.idx}
            </div>
            <div>
