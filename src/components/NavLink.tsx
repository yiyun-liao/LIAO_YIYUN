...
export function NavLink({ id, underline = false, icon: Icon, onClick, children }: NavLinkProps) {
  return (
    <button
      onClick={() => onClick?.(id)}
      className={`font-mono flex items-center p-2 border-b text-[12px] font-medium tracking-[.12em] uppercase opacity-75 hover:opacity-100 hover:text-[#2467D2] hover:font-bold
